...
document.addEventListener("DOMContentLoaded", function () {
    ...
    baseImageBlack.onload = function () {
        ...
        const nextPlayercolor = document.querySelector("#nextPlayercolor");
        ...
        function newGame() {
            ...
            p1Chip = new Chip(30, btnP1Color.value, inpP1name.value, baseImageBlack)
            p2Chip = new Chip(30, btnP2Color.value, inpP2name.value, baseImageWhite)
            nextPlayerSelector.innerText = inpP1name.value
            nextPlayercolor.style = 'background-color:' + btnP1Color.value;
        ...
        function dropping(e) {
            ...
            dropzones.dropZones.forEach(dropZone => {
                if (dropZone.isInside(e.offsetX, e.offsetY)) {
                    if (xCounter(dropZone.columnNumber) < game.matrix[0].length) {
                        gameBoard.highlightColumn(dropZone.columnNumber)
                        game.insertChip(dropZone.columnNumber, new Chip(30, nextChip.color, nextChip.playerName, nextChip.chipImage))
                        gameBoard.highlightColumn(8)
                        gameBoard.drawBoard(game)
                        setNextTurn()
                    }
            ...
            if (game.finished) {
                ...
                gamingDiv.style = 'display:none;'
                nextPlayercolor.style = 'background-color:' + game.winner[0].color;
        ...
        function setNextTurn() {
            ...
            nextPlayercolor.style = 'background-color:' + next.color;
            nextPlayerSelector.innerText = next.playerName
        }
    };
